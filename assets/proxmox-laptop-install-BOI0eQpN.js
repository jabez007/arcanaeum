...
<hr>
<h2>Step 6: The Moment of Truth (First Login)</h2>
<p>The installation is finished, the USB drive is pulled out, and you’ve rebooted. Instead of a desktop, you’re greeted by a stark, black-and-white Linux login prompt.</p>
<p><strong>Congratulations! Your server is officially alive.</strong></p>
<p>But we don’t do our work at the physical console. We do it from the comfort of another machine:</p>
<ol>
<li><strong>The URL</strong>: Open a browser on your main computer and navigate to <code>https://your-proxmox-ip:8006</code>. (Make sure you include the <code>https</code> and the <code>:8006</code> port!)</li>
<li><strong>The “Scary” Warning</strong>: Your browser will scream that the connection is not private. This is because Proxmox uses a self-signed SSL certificate. Click “Advanced” and “Proceed” — it’s perfectly safe on your local network.</li>
<li><strong>The Login</strong>: Use the username <code>root</code> and the password you created during Step 4.</li>
<li><strong>The “No Subscription” Nag</strong>: Immediately after logging in, you’ll see a warning about not having a valid subscription. Don’t worry, Proxmox isn’t broken. It’s just reminding you that you’re using the free version.</li>
</ol>
<p><strong>Wait, where are the updates?</strong>
Normally, the very next step is to swap the enterprise repositories for the community ones and run a full system update (<code>apt update &amp;&amp; apt upgrade</code>). However, because we’re on a laptop without an Ethernet port, we have a bigger problem: <strong>we aren’t even online yet.</strong></p>
<p>We’ll tackle the “No Subscription” fix and the system updates in the next post, once we’ve performed some networking surgery to get this laptop on Wi-Fi.</p>
<blockquote>
<p>[!WARNING] &gt; <strong>The Laptop Trap:</strong>
Whatever you do, <strong>do not close the lid yet.</strong> By default, Proxmox will treat a lid-close as a command to go to sleep, which will kill your network connection and make the Web UI unreachable. Check out my “Laptop Lid Interlude” post to fix this before you put your server in its permanent home.</p>
</blockquote>
<hr>
...
`,s={id:t,slug:e,frontmatter:o,readingTime:n,content:r};export{r as content,s as default,o as frontmatter,t as id,n as readingTime,e as slug};
