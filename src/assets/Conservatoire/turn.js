...
export default function ($) {
  ...
  var turnMethods = {
    ...
    // Define constants
      ...
      let i; const that = this; let pageNum = 0; const data = this.data(); const
        ch = this.children();
    ...
    center(page) {
      ...
      if (!data.noCenter) {
        if (data.display == 'double') {
          ...
          } else if (!view[0]) left += size.width / 4;
          else if (!view[1]) left -= size.width / 4;
        }
    ...
    size(width, height) {
      ...
      if (data.pageObjs[0]) data.pageObjs[0].css({ width: pageWidth, height });
    ...
    pages(pages) {
      ...
      if (pages) {
        if (pages < data.totalPages) {
          for (let page = data.totalPages; page > pages; page--) this.turn('removePage', page);
        }
      ...
      switch (data.effect) {
        ...
        case 'sheet':
          ...
          if (full) {
            ...
            if (turnData.opts.gradients) data.ashadow.css({ width, height });

            if (flipMethods._backGradient.call(this)) data.bshadow.css({ width, height });
          }
        ...
        case 'sheet':
          ...
          var compute = function () {
            ...
            if (turnData.opts.gradients) {
              ...
              const far = Math.sqrt(Math.pow(endingPoint.x - point.x, 2) + Math.pow(endingPoint.y - point.y, 2)) / width;
  ...
  function dec(that, methods, args) {
    ...
    if (methods[args[0]]) return methods[args[0]].apply(that, Array.prototype.slice.call(args, 1));
    ...
    animatef(point) {
      ...
      if (point) {
        ...
        var frame = function () {
          ...
          for (let i = 0; i < len; i++) v.push(data.effect.easing(1, timeDiff, point.from[i], diff[i], point.duration));
